...
use crate::{
    pb::{
        notification_server::NotificationServer, send_request::Message, EmailMessage, SendRequest,
        SendResponse,
    },
        ...
        tokio::spawn(async move {
            while let Some(Ok(req)) = stream.next().await {
                ...
                let res = match req.message {
                    Some(Message::Email(email)) => email.send(notif_clone).await,
                    Some(Message::Sms(sms)) => sms.send(notif_clone).await,
                    Some(Message::InApp(in_app)) => in_app.send(notif_clone).await,
                    None => {
...
impl SendRequest {
    pub fn new(
        ...
        let msg = Message::Email(EmailMessage {
...
fn dummy_send() -> mpsc::Sender<Message> {
